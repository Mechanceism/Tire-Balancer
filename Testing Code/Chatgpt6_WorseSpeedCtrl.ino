...
const float alpha = 0.00;  // Balance between gyro & accel
...
void controlActuator(int upPWM, int downPWM, float output) {
  ...
  int speed = map(abs(output*output*output), 0, 10, 0, 150);
  speed = constrain(speed, 15, 255);  // Ensure speed stays within limits
...
// Function to check serial input for "START"
void checkSerialInput() {
    if (Serial.available() > 0) {
        ...
        if (input.equalsIgnoreCase("START")) {
            requestBalance = true;
            stableCount = 0;  // Reset stability counter when START is received
            //Serial.println("Balance requested.");
        }
        ...
        if (input.equalsIgnoreCase("debugon")) {
            doDebug = true;
        }
        if (input.equalsIgnoreCase("debugoff")) {
            doDebug = false;
        }
    }
}
