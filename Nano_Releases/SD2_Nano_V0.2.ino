...
// start Level (Initialize)
  ...
  float minSpeed = 50; // Minimum speed of actuators
  float maxSpeed = 100; // Maximum speed of actuators
  float rollOffset = 0;
  float pitchOffset = 2;
...
void LaserPosition(){
  if (WeightAngle <= 180) {
  pulseWidth = map(WeightAngle, 0,180,degree0,degree180);
  digitalWrite(Laser1, HIGH);
  digitalWrite(Laser2, LOW);
  } else {
  pulseWidth = map(WeightAngle-180, 0,180,degree0,degree180);
  digitalWrite(Laser1, LOW);
